...
> [!TIP]
> **Stream freezes or drops? Match the symptom to the fix:**
>
> - **Freezes for a few seconds, then resumes or fails over** - the upstream stall outlasted the shared buffer, which only bridges `BUFFER_CHUNK_NUM x 1 MiB / stream bitrate` seconds (defaults: ~22s for a 6 Mbps channel). Raise `BUFFER_CHUNK_NUM` or lower `STREAM_TIMEOUT` so failover fires before the buffer runs dry.
...
| SORTING_KEY                                          | Set tag to be used for sorting the stream list. Default `provider-order` keeps each provider's own channel order (earlier M3U index first); set `title` for alphabetical                                          | provider-order                                                | title, tvg-id, tvg-chno, tvg-group, tvg-type, source, provider-order                                                                                                                                                                                  |
